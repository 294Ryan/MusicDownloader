#---------v1.1更新內容---------#
#v1.1 新增了處理yt網址出現&list=...會把整個播放清單都下載的問題
#v1.1 會找到出現"&"的索引 並只取其索引前的網址作為下載用的網址(只下載1個檔案)
...
def download():
    ...
    if index==-1:  #v1.1新增
        ...
        url=url[0:index]  #v1.1新增
        print(f"new url is : {url}")  #v1.1新增
    options = {
        'format': 'bestaudio/best',
        'outtmpl': os.path.join(download_folder, '%(title)s.%(ext)s'),  #儲存位置與檔名
        'ffmpeg_location': r'C:\ffmpeg\bin',  #ffmpeg 路徑
        'postprocessors': [{
